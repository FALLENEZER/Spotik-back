...
//    }
    ...
    public function playback(Room $room, Request $request): JsonResponse
    {
        ...
        $data = json_decode($request->getContent(), true);
        $this->service->togglePLayback($room, $user, $data);

        return $this->json(['status' => 'ok']);
    }

    #[Route('/signal', name: 'signal', methods: ['POST'])]
    public function signal(Room $room, Request $request): JsonResponse
    {
        $user = $this->getUser();
        if (!$user) {
            return $this->json(['error' => 'Unauthorized'], 401);
        ...
        $data = json_decode($request->getContent(), true);
        $this->service->sendWebRTCSignal($room, $user, $data);
